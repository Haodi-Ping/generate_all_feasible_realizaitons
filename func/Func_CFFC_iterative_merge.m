function [r_merge, flag, ID_merge] = Func_CFFC_iterative_merge...
...
num_merge = size(ID_merge,2);
conn_merge = ConnectivityM(ID_merge,ID_merge);
dist_merge = distMatrix(ID_merge,ID_merge);
neighbor_merge = connectivity2neighbors(conn_merge);
patchCoordMatrix = patchLocalization...
    (num_merge, neighbor_merge, dist_merge, conn_merge);
AAAP_Pos = aaap(num_merge, neighbor_merge, patchCoordMatrix);
ARAP_Pos = arap(num_merge, AAAP_Pos, neighbor_merge, patchCoordMatrix);
...
SubgraphCoord_init(1:2,ID_1)  = r_l';
SubgraphCoord_init(3:4,ID_2)  = r_r_transform';
subgraph = zeros(2,num_merge);
subgraph(1,ind_1) = 1;
subgraph(2,ind_2) = 1;
ratio = Func_Ratio(subgraph,dist_merge,conn_merge);
pos_merge_init =  Func_Iterative_Merge(ARAP_Pos,neighbor_merge, patchCoordMatrix,...
    subgraph,SubgraphCoord_init,ratio);
% r_merge_1(ind_1,:) = r_l;
...
SubgraphCoord_flip(1:2,ID_1)  = r_l';
SubgraphCoord_flip(3:4,ID_2)  = r_r_flip';
pos_merge_flip =  Func_Iterative_Merge(ARAP_Pos,neighbor_merge, patchCoordMatrix,...
    subgraph,SubgraphCoord_flip,ratio);
% r_merge_2(ind_1,:) = r_l;
...
%% save result
r_merge{1} = pos_merge_init;
r_merge{2} = pos_merge_flip;
flag = [flag_1;flag_2];
end